...
const updateMetaTags = (song: Song) => {
    ...
    const title = `${song.title} - ${song.creator || '未知创作者'} | ACE-Step UI`;
    const description = `收听《${song.title}》 - ${song.style}。播放 ${song.viewCount || 0} 次，点赞 ${song.likeCount || 0} 次。使用 ACE-Step UI 创作你的 AI 音乐。`;
    ...
    updateOrCreateMeta('meta[property="music:musician"]', 'content', song.creator || '未知创作者');
};

const resetMetaTags = () => {
    document.title = 'ACE-Step UI - 本地 AI 音乐生成平台';
    const defaultDescription = '在本地使用 AI 创作原创音乐，支持多种风格、自定义歌词与高质量生成。';
    ...
    updateMeta('meta[property="og:title"]', 'ACE-Step UI - 本地 AI 音乐生成平台');
    ...
    updateMeta('meta[name="twitter:title"]', 'ACE-Step UI - 本地 AI 音乐生成平台');
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            >
                                <div className="w-6 h-6 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-xs font-bold text-white overflow-hidden">
                                    {song.creator_avatar ? (
                                        <img src={song.creator_avatar} alt={song.creator || '创作者'} className="w-full h-full object-cover" />
                                    ) : (
                                        song.creator ? song.creator[0].toUpperCase() : 'A'
                                    )}
                                </div>
                                <span className="text-zinc-900 dark:text-white font-semibold">{song.creator || '匿名用户'}</span>
                            </div>
                            {song.singerName && (
                                <span className="rounded-full bg-orange-100 px-2 py-1 text-xs font-medium text-orange-700 dark:bg-orange-500/15 dark:text-orange-300">
                                    虚拟歌手：{song.singerName}
                                </span>
                            )}
                        </div>
                        ...
                        <div className="text-xs text-zinc-500">
                            创建于 {new Date(song.createdAt).toLocaleDateString('zh-CN', { year: 'numeric', month: 'long', day: 'numeric' })} {new Date(song.createdAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}
                            {!song.isPublic && song.userId === user?.id && (
                                <span className="ml-2 px-2 py-0.5 bg-zinc-200 dark:bg-zinc-800 rounded text-xs text-zinc-600 dark:text-zinc-400">私密</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-4 md:space-y-6">
                        ...
                        {song.lyrics && (
                            <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4">
                                <h3 className="text-sm font-semibold text-zinc-900 dark:text-white mb-3">歌词</h3>
