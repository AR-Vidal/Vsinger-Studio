...
import { Play, MoreHorizontal, Heart, ListPlus, Pause, Search, Filter, Check, Lock, Loader2, ThumbsUp, Video, Info, Clock } from 'lucide-react';
                    ...
                    {listItems.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-64 text-zinc-500 space-y-4 border border-dashed border-zinc-200 dark:border-white/5 rounded-2xl bg-zinc-50 dark:bg-white/[0.02]">
                            ...
                            <button
                                ...
                                className="vs-accent-text text-sm font-bold hover:underline"
                            >
                                {t('clearFilters')}
                            </button>
                        </div>
    ...
    return (
        ...
        >
            {isSelectionMode && (
                <button
                    ...
                    className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isChecked
                            ? 'bg-[#052659] border-[#052659] text-white'
                    ...
                    <div className="absolute inset-0 bg-black/40 flex flex-col items-center justify-center gap-1">
                        {song.queuePosition ? (
                            ...
                            <div className="flex items-end gap-1 h-6">
                                <div className="w-1 bg-[#5483B3] rounded-full music-bar-anim" style={{ animationDelay: '0.0s' }}></div>
                                <div className="w-1 bg-[#5483B3] rounded-full music-bar-anim" style={{ animationDelay: '0.2s' }}></div>
                                <div className="w-1 bg-[#5483B3] rounded-full music-bar-anim" style={{ animationDelay: '0.4s' }}></div>
                                <div className="w-1 bg-[#5483B3] rounded-full music-bar-anim" style={{ animationDelay: '0.1s' }}></div>
                            </div>
                        )}
                    </div>
                ) : (
                ...
                <div className="space-y-1">
                    <div className="flex items-center gap-2">
                        {isEditingTitle && isOwner ? (
                            <input
                                ...
                                className="vs-accent-focus font-bold text-lg bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded border border-[rgba(84,131,179,0.45)] text-zinc-900 dark:text-white min-w-0 flex-1"
                            />
                        ) : (
                            <h3
                                className={`font-bold text-lg truncate ${isCurrent ? 'vs-accent-text' : 'text-zinc-900 dark:text-white'} ${isOwner && !song.isGenerating ? 'cursor-pointer hover:underline' : ''}`}
                                onClick={(e) => {
                        ...
                        <span className="vs-gradient-button inline-flex items-center justify-center px-1.5 py-0.5 text-[9px] font-bold text-white rounded-sm shadow-sm" title={`DiT model: ${song.ditModel || 'undefined'}`}>
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1.5">
                            <div className="vs-gradient-icon w-4 h-4 rounded-full text-[8px] flex items-center justify-center font-bold text-white">
                    ...
                    {song.isGenerating && (
                        <div className="pt-2">
                            <div className="h-1 rounded-full bg-zinc-200/70 dark:bg-white/10 overflow-hidden">
                                <div
                                    className={`vs-accent-progress h-full transition-all ${song.progress === undefined ? 'opacity-40' : ''}`}
                                    style={{
                                        width: `${Math.min(
                                            100,
            ...
            <div className="text-xs font-mono text-zinc-500 dark:text-zinc-600 self-start pt-1">
                {song.isGenerating ? (
                    <span className={song.queuePosition ? 'text-amber-500' : 'vs-accent-text'}>
